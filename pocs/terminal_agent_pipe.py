...
class AgentOutput:
    def __init__(self):
        self.ai_output = ""
        self.break_line_threshold = 100

    def ai_stream_callback(self, x):
        if "\n" in x or len(self.ai_output) > self.break_line_threshold:
            self.ai_output += x
            print(colorama.Fore.MAGENTA, self.ai_output.strip())
            self.ai_output = ""
        else:
            self.ai_output += x
...
if __name__ == "__main__":
    ...
    agent.set_ai_stream_callback(ai_output.ai_stream_callback)
